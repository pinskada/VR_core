...
class TCP:
    ...
    receive_loop_interval = 0.001   # Interval for receiving messages (in seconds)

    google_dns = "8.8.8.8" # Used to check if the device is connected to the internet
    http_port = 80 # Port for HTTP requests (if needed)

    max_resend_attempts = 3      # Number of times to resend a message if not acknowledged

    # Timeout for establishing a connection, where -1 means no timeout (in seconds)
    connect_timeout = 60

    max_packet_size = 0xFFFFFF  # Maximum allowed packet size
...
class Tracker:
    ...
    blink_calibration_L = "blink_calibration/blink_calibration_cropL.npy"
    blink_calibration_R = "blink_calibration/blink_calibration_cropR.npy"

    # Importer name for the EyeLoop process
    importer_name = "shared_memory_importer"

    # Interval for health check of the eyeloop processes in seconds
    eyeloop_health_check_interval = 3
