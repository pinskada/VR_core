...
class TrackerLauncher:
    ...
    def __init__(self, test_mode=False):
        ...
        if module_list.queue_handler:
            self.command_queue_L, self.command_queue_R = module_list.queue_handler.get_command_queues()
            self.response_queue_L, self.response_queue_R = module_list.queue_handler.get_response_queues()
        ...
        try:
            self.proc_left = Process(
                target=run_eyeloop,
                args=("Left",
                      tracker_config.importer_name,
                      tracker_config.sharedmem_name_left,
                      tracker_config.blink_calibration_L,
                      self.command_queue_L,
                      self.response_queue_L,
                      self.sync_queue_L,
                      self.acknowledge_queue_L,
                      test_mode
                )
            )
            self.proc_right = Process(
                ...
                args=("Right",
                      tracker_config.importer_name,
                      tracker_config.sharedmem_name_right,
                      tracker_config.blink_calibration_R,
                      self.command_queue_R,
                      self.response_queue_R,
                      self.sync_queue_R,
                      self.acknowledge_queue_R,
                      test_mode
                    )
            )
        except Exception as e:
            if module_list.health_monitor:
                module_list.health_monitor.failure("TrackerLauncher", \
                f"Failed to initialize Eyeloop processes: {e}")
        ...
        # Allow some time for the processes to stabilize
        time.sleep(tracker_config.process_launch_time)
    ...
    def _eyeloop_monitor(self):
        ...
        while True:
            if not self.proc_left.is_alive() and self.left_alive:
                if module_list.health_monitor:
                    module_list.health_monitor.failure("Left EyeLoop", "Process is not responding.")
            ...
            if not self.proc_right.is_alive() and self.right_alive:
                if module_list.health_monitor:
                    module_list.health_monitor.failure("Right EyeLoop",
    ...
    def is_online(self):
        """Checks if both Eyeloop processes are online."""
        return self.online and self.left_alive and self.right_alive


    def wait(self):
        """Waits for both Eyeloop processes to finish."""
        self.proc_left.wait()
        self.proc_right.wait()

    def stop(self):
        """Stops both Eyeloop processes."""
        if module_list.frame_provider:
            module_list.frame_provider.cleanup()
